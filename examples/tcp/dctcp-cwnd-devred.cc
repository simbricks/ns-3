...
static void
CwndChangeS0 (int interval, Time measurementWindow, uint32_t oldCwnd, uint32_t newCwnd)
{
  //NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << "\t" << newCwnd);
  
  if (Simulator::Now().GetSeconds() >= measurementWindow.GetSeconds()){
    int ts =  Simulator::Now ().ToInteger (Time::MS);

    if ( cwnd_t0 == 0 || ts - cwnd_t0 >= interval){
      cwnd_t0 =  Simulator::Now ().ToInteger (Time::MS);
      stream0 << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
...
static void
CwndChangeS1 (int interval, Time measurementWindow, uint32_t oldCwnd, uint32_t newCwnd)
{
  //NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << "\t" << newCwnd);
  ...
  if (Simulator::Now().GetSeconds() >= measurementWindow.GetSeconds()){
    ...
    if ( cwnd_t1 == 0 || ts - cwnd_t1 >= interval){
      cwnd_t1 =  Simulator::Now ().ToInteger (Time::MS);
      stream1 << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
...
/*static void
CwndChangeS1 (Ptr<OutputStreamWrapper> stream, uint32_t oldCwnd, uint32_t newCwnd)
{
  //NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << "\t" << newCwnd);
...
void
CheckT1QueueSize (Ptr<Queue<Packet>> queue)
{
  // 1500 byte packets
  ...
  t1QueueLength << std::fixed << std::setprecision (2) << Simulator::Now ().GetSeconds () << " " << qSize << " " << std::endl;
...
int main (int argc, char *argv[])
{
  ...
  Time measurementWindow = Seconds (5);
  bool enableSwitchEcn = true;
  Time progressInterval = MilliSeconds (100);
  int cwndInterval = 20; // millisecond
  size_t num_pairs = 2;
  ...
  //LogComponentEnable ("DevRedQueuePPP", LOG_LEVEL_ALL);
  ...
  pointToPointT.SetQueue("ns3::DevRedQueuePPP", "MaxSize", QueueSizeValue (QueueSize (QueueSizeUnit::BYTES, 4000000)));
  pointToPointT.SetQueue("ns3::DevRedQueuePPP", "MinTh", DoubleValue (EcnTh));
  pointToPointT.SetQueue("ns3::DevRedQueuePPP", "MeanPktSize", DoubleValue (mtu));
  pointToPointT.SetDeviceAttribute ("Mtu", UintegerValue(mtu));
  pointToPointT.SetChannelAttribute ("Delay", TimeValue (linkLatency));
...
*/
  ...
  st0 << "dctcp-modes-s0-" << mtu << "-" << EcnTh << "-" << rtt << "us.cwnd";
  st1 << "dctcp-modes-s1-" << mtu << "-" << EcnTh << "-" << rtt << "us.cwnd";
  std::string f_st0 = st0.str ();
  std::string f_st1 = st1.str ();
  stream0.open(f_st0, std::ios::out);
  stream1.open(f_st1, std::ios::out);
  ...
  sockets[0]->TraceConnectWithoutContext ("CongestionWindow",
                                            MakeBoundCallback (&CwndChangeS0, cwndInterval, flowStartupWindow + convergenceTime));
  ...
  sockets[1]->TraceConnectWithoutContext ("CongestionWindow",
                                         MakeBoundCallback (&CwndChangeS1, cwndInterval, flowStartupWindow + convergenceTime));

  std::ostringstream tput;
  std::ostringstream qlen;
  tput << "dctcp-modes-tput-" << mtu << "-" << EcnTh << "-" << rtt << "us.dat";
  qlen << "dctcp-modes-qlen-" << mtu << "-" << EcnTh << "-" << rtt << "us.dat";
  ...
  t1QueueLength << "#Time(s) qlen(byte) qlen(us)" << std::endl;
  ...
  Ptr<PointToPointNetDevice> track_dev = T1T2.Get(1)->GetObject<PointToPointNetDevice> ();
  Simulator::Schedule (flowStartupWindow + convergenceTime, &CheckT1QueueSize, track_dev->GetQueue());
