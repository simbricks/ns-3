...
#define BOOT_TIME 1
#define CONV_WIN 1
#define MEASURE_WIN 5.0

// #define PCAP_ENABLE // Use Csma devices to generate pcap files

using namespace ns3;
NS_LOG_COMPONENT_DEFINE("Tput_FatTreeC_Example");
...
void
PrintThroughput (Time measurementWindow, size_t id)
{
    ...
    int host_idx, int num_pod, int racks_per_pod, int num_hosts_per_rack)
{
    ...
    result.pod = host_idx / (racks_per_pod * num_hosts_per_rack);
    result.rack = (host_idx / num_hosts_per_rack) % racks_per_pod;
    result.host = host_idx % num_hosts_per_rack;
...
void
TraceSink(int id, Ptr<Packet const> packet, const Address& address)
{
...
void
sink(ns3::Ipv4Address add, ns3::Ptr<Node> node, int id)
{
    ...
    sinkApp.Get(0)->TraceConnectWithoutContext("Rx", MakeBoundCallback(&TraceSink, id));
    sinkApp.Start(Seconds(START + CONV_WIN));
    sinkApp.Stop(Seconds(END));
}

void
client(ns3::Ipv4Address add, ns3::Ptr<Node> node, int flow_size, int server_id)
{
    ...
    uv->SetAttribute("Max", DoubleValue(0.002));
...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("Tput_FatTreeC_Example",
    ...
    DataRate spine_agg_linkRate("40Gb/s");
    DataRate agg_tor_linkRate("40Gb/s");
    ...
    int num_hosts_per_rack = 8;

    int total_hosts = k_value * racks_per_pod * num_hosts_per_rack;
    ...
    sink_start.resize(num_dum_hosts/2, 0);
    ...
    for (int i = 0; i < num_detail_host_ser; i++)
    {
        ...
        int cli_idx = i + total_hosts / 2;
        detailHostPairs[cli_idx / total_racks].num_detail_cli++;
    }
    ...
    for (int i = 0; i < num_pod; i++){
        for (int j = 0; j < racks_per_pod; j++){
            ...
            int dum_start_idx = detailHostPairs[rack_idx].num_detail_ser + detailHostPairs[rack_idx].num_detail_cli;
    ...
    for (int i = 0; i < num_pod; i++){
        for (int j = 0; j < racks_per_pod; j++){
            ...
            for (int k = 0; k < num_hosts_per_rack; k++){
                ...
                int host_idx = i * racks_per_pod * num_hosts_per_rack + j * num_hosts_per_rack + k;
                // NS_LOG_INFO("Host " << host_idx << " connected to pod " << i << " rack " << j << " host " << k);
                if ((host_idx < num_detail_host_ser) || (host_idx >= total_hosts/2 && host_idx < total_hosts/2 + num_detail_host_cli)){
                    ...
                    if (host_idx < total_hosts / 2){
                        ...
                        //client
                        detail_host_idx = host_idx - total_hosts / 2 + num_detail_host_ser;
                    }
    ...
    for (int i = 0; i < num_pod; i++){
        for (int j = 0; j < racks_per_pod; j++){
            int dum_start_idx = detailHostPairs[i * racks_per_pod + j].num_detail_ser + detailHostPairs[i * racks_per_pod + j].num_detail_cli;
            ...
            for (int k = 0; k < num_hosts_per_rack; k++){
                ...
                if (host_idx >= num_detail_host_ser && host_idx < total_hosts/2 ){
                    ...
                    // Client
                    int client_idx = host_idx + total_hosts / 2;
                    struct HostPos client_pos = GetPodRackHostFromHostIdx(client_idx, num_pod, racks_per_pod, num_hosts_per_rack);
                    int dum_start_idx_cli = detailHostPairs[client_pos.pod * racks_per_pod + client_pos.rack].num_detail_cli;
    ...
    Simulator::Schedule (Seconds(BOOT_TIME + CONV_WIN), &InitializeCounters, num_dum_hosts/2);
    Simulator::Schedule (Seconds(BOOT_TIME + CONV_WIN + MEASURE_WIN), &PrintThroughput, Seconds(MEASURE_WIN), num_dum_hosts/2);
    
    GlobalValue::Bind("ChecksumEnabled", BooleanValue(true));
    Simulator::Schedule(Seconds(0.0), &PrintSimProgress);
    Simulator::Schedule(Seconds(START + CONV_WIN), &PrintClientStart);
